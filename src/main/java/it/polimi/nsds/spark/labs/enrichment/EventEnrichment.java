...
public class EventEnrichment {
    public static void main(String[] args) throws TimeoutException, IOException {
        ...
        productClassificationFields.add(DataTypes.createStructField("product", DataTypes.IntegerType, false));
        ...
        final StreamingQuery query= inStreamDF.alias("streaming")
                .join(productsClassification.alias("classification"),
                        col("streaming.product").equalTo(col("classification.product")),
                        "left_outer")
                .groupBy(col("classification.classification"))
                .count().writeStream()
                .outputMode("update")
                .format("console")
                .start()
