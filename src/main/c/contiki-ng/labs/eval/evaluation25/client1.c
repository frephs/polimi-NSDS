...
PROCESS_THREAD(udp_client_process, ev, data) {
  ...
  while (1) {
    ...
    if (NETSTACK_ROUTING.node_is_reachable() &&
      ...
      if (state == 0) {
        send_lock(&dest_ipaddr, 2); // Scenario 1
      } else if (state == 1) {
        send_write(&dest_ipaddr, 8, 0); // Scenario 1
      } else if (state == 2) {
        send_write(&dest_ipaddr, 10, 1); // Scenario 2
      } else if (state == 3) {
        send_read(&dest_ipaddr, 10); // Scenario 3
      } else if (state == 4) {
        send_lock(&dest_ipaddr, 3); // Scenario 4
      } else if (state == 5) {
        send_write(&dest_ipaddr, 7, 0); // Scenario 4
      } else if (state == 6) {
        send_lock(&dest_ipaddr, 3); // Scenario 5
      } else if (state == 7) {
        send_write(&dest_ipaddr, 7, 0); // Scenario 5
      }
